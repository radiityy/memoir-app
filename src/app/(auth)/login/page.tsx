...
export default function LoginPage() {
  ...
  async function handleSubmit() {
    ...
    if (mode === 'register') {
      ...
      if (error) setError(error.message)
      else setSuccess('Link reset password sudah dikirim ke emailmu.')
    }
    ...
    <div className="min-h-screen bg-[#f7f4ef] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white border border-[#e0d9ce] rounded-2xl p-6 flex flex-col gap-4">
          ...
          {mode === 'register' && (
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-[#888780] uppercase tracking-wide">Konfirmasi Password</label>
              <input
                type="password"
          ...
          >
            {loading ? 'loading...' : mode === 'login' ? 'Masuk' : mode === 'register' ? 'Buat akun' : 'Kirim link reset'}
          </button>

          <div className="flex flex-col items-center gap-2 pt-1">
            {mode === 'login' && (
              <>
                <button onClick={() => { setMode('register'); setError('') }} className="text-xs text-[#888780] hover:text-[#1a1a18] transition-colors">
                  belum punya akun? <span className="underline underline-offset-2 text-[#1a1a18]">daftar</span>
                </button>
                <button onClick={() => { setMode('reset'); setError('') }} className="text-xs text-[#B4B2A9] hover:text-[#888780] transition-colors">
                  lupa password?
                </button>
              </>
            )}
            {mode === 'register' && (
              <button onClick={() => { setMode('login'); setError('') }} className="text-xs text-[#888780] hover:text-[#1a1a18] transition-colors">
                sudah punya akun? <span className="underline underline-offset-2 text-[#1a1a18]">masuk</span>
              </button>
            )}
            {mode === 'reset' && (
              <button onClick={() => { setMode('login'); setError('') }} className="text-xs text-[#888780] hover:text-[#1a1a18] transition-colors">
